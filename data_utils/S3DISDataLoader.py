...
class S3DISDataset(Dataset):
    ...
    # def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=1, block_size=1000.0, sample_rate=1.0, transform=None):
        ...
        # rooms = [room for room in rooms if 'pArea_' in room]
        if split == 'train':
            rooms_split = [room for room in rooms if not 'Area_{}'.format(test_area) in room]
            # rooms_split = [room for room in rooms if not 'pArea_{}'.format(test_area) in room]
        else:
            rooms_split = [room for room in rooms if 'Area_{}'.format(test_area) in room]
            # rooms_split = [room for room in rooms if 'pArea_{}'.format(test_area) in room]
...
class ScannetDatasetWholeScene():
    ...
    def __init__(self, root, block_points=4096, split='test', test_area=3, stride=500, block_size=1000.0,padding=1):
        ...
        if self.split == 'train':
            ...
            self.file_list = [d for d in os.listdir(root) if d.find('pArea_%d' % test_area) is -1]
        else:
            # self.file_list = [d for d in os.listdir(root) if d.find('Area_%d' % test_area) is not -1]
            self.file_list = [d for d in os.listdir(root) if d.find('pArea_%d' % test_area) is not -1]
...
if __name__ == '__main__':
    ...
    data_root = '/data/s3dis/p_indoor3d/'
    num_point, test_area, block_size, sample_rate = 4096, 5, 1.0, 0.01
    # num_point, test_area, block_size, sample_rate = 4096, 1, 1000.0, 0.01
