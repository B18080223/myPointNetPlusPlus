...
DATA_PATH = os.path.join(ROOT_DIR, 'data','s3dis', 'pArea')
# g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))]
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/pclass_names.txt'))]
...
#                  'board':       [200,200,200],
#                  'clutter':     [50,50,50]}
g_class2color = {'plant':  [0,255,0],
                 'others': [204,153,102]}
# g_easy_view_labels = [7,8,9,10,11,1]
# g_easy_view_labels = [0,1]
g_easy_view_labels = [0]
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    for idx in range(len(xbeg_list)): 
       xbeg = xbeg_list[idx]
       ybeg = ybeg_list[idx]
       xcond = (data[:,0]<=xbeg+collect_point_label) & (data[:,0]>=xbeg)
